...
namespace IoTSharp.Controllers
{
    ...
    public partial class WeChatUserAccountController : ControllerBase
    {
        ...
        public WeChatUserAccountController(
            UserManager<IdentityUser> userManager,
            SignInManager<IdentityUser> signInManager,
            IConfiguration configuration, ILogger<WeChatUserAccountController> logger, ApplicationDbContext context,
            IOptions<AppSettings> options,
            IQinglanApi qinglanApi
            )
        {
        ...
        public async Task<ApiResult<PagedData<UserItemDto>>> List([FromQuery] UserQueryDto m)
        {
            ...
            try
            {
                var list = new List<UserItemDto>();

                var weChatUserList = await _qinglanApi.GetWeChatUserListAsync(m.Offset, m.Limit);

                if (weChatUserList.Code == 200)
                {
                    foreach (var weChatUser in weChatUserList.Rows)
                    {
                        var user = new UserItemDto()
                        {
                            UserName = weChatUser.Nickname,
                            PhoneNumber = weChatUser.Mobile
                        };
                        list.Add(user);
                    }
            ...
            {
                return new ApiResult<PagedData<UserItemDto>>(ApiCode.Exception, e.Message, null);
            }

            return new ApiResult<PagedData<UserItemDto>>(ApiCode.Success, "OK", new PagedData<UserItemDto>
            {
                total = 0,
                rows = new List<UserItemDto>()
            });
        }
